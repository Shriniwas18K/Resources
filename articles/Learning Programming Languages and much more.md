...
Read and learn language only from its documentation.It takes some time but it is the source and gives deep understanding of concepts. 
...
- For any product we have its UI in Javascript as scripting language adding dynamic user interaction to the UI built in markup languages like XML,XAML,HTML.
- And in the product we have server builtin or browser builtin which hosts the internal webpages or UI page(applets,hta,htm files) which runs the javascript.
- The internal product code handles events and in response renders diffirent things on UI.
- Also we have internal serialisations and data streaming and http client communication going on for data flow which involves message queues like rabbitmq,kafka,protobuf.
- Jenkins or Azure devops pipelines run with automated building code of product, and running tests in it, 
- Product interaction is by two ways, direct UI clicks which can be automated using selenium, directly use APIs which can be automated by writing tests in the language in which APIs are exposed.
